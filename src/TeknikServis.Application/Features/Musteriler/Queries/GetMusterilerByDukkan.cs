...
// Request
public record GetMusterilerByDukkanRequest(
    string? Arama,
    int Page = 1,
    int PageSize = 20
);
...
// Query
public record GetMusterilerByDukkanQuery(GetMusterilerByDukkanRequest Data) : IRequest<Result<PaginatedResponse<MusteriResponse>>>;

// Handler
public class GetMusterilerByDukkanHandler : IRequestHandler<GetMusterilerByDukkanQuery, Result<PaginatedResponse<MusteriResponse>>>
{
    private readonly IApplicationDbContext _context;
    private readonly ICurrentUserService _currentUserService;

    public GetMusterilerByDukkanHandler(IApplicationDbContext context, ICurrentUserService currentUserService)
    {
        _context = context;
        _currentUserService = currentUserService;
    }

    public async Task<Result<PaginatedResponse<MusteriResponse>>> Handle(GetMusterilerByDukkanQuery query, CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrEmpty(request.Arama))
        {
            queryable = queryable.Where(x =>
                ...
                (x.AdSoyad != null && x.AdSoyad.Contains(request.Arama)) ||
                (x.FirmaAdi != null && x.FirmaAdi.Contains(request.Arama)));
        }
